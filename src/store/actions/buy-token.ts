...
export const verifyMaxCanBuy = async (
    campaignId: string,
    unit: string,
    amount: any,
    tokenConvert: any,
  ...
  if (campaignContract) {
    ...
    const {tokenDecimals, erc20ConversionRate, ethRate} = campaignDetailStore;
      ...
      if (contract) {
        if (unit === 'eth') {
          ...
          if (buyResult) {
            ...
            await baseRequest.post(apiRoute(apiRoute(`/transaction-create`)), {
          ...
          const USDT_TOKEN = process.env.REACT_APP_SMART_CONTRACT_USDT_ADDRESS as string;
           ...
           if (ercContract) {
             ...
             if (buyResult) {
               ...
               const baseRequest = new BaseRequest();
               await baseRequest.post(apiRoute(`/transaction-create`), {
                 campaign_hash: campaignId,
                 transaction_hash: buyResult.transactionHash,
                 eth: 0,
                 usdt: amount,
