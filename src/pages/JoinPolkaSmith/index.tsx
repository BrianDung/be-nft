...
const ksmDecimals = 1_000_000_000_000
const parachanID = 2009
const poolSize = 10_500_000
const isDue = (new Date("2021-06-19T23:59:59Z").getTime()) < (new Date().getTime())
...
const JoinPolkaSmith = (props: any) => {
    ...
    const getBalance = async (address: any) => {
        ...
        api.query.system.account(address).then(account => {
            ...
            // @ts-ignore
            return setKsmBalance({total: parseFloat((account.data.free / ksmDecimals).toString(10)), unlocked: parseFloat((account.data.free / ksmDecimals).toString(10))
            });
        })
    }
    ...
    const submitContribution = async () => {
        dispatch(alertFailure('Please wait to next round'))
        return
        ...
        // setStatusMessage("Waiting for sign the transaction on Polkadot{.js} extension")
        // const countTimeFunc = countExecuteTime()
        // // @ts-ignore
    ...
    const getContribution = async (address: any) => {
        ...
        contributions.forEach(val => {
            totalK += BigInt(val.data[0].toString())
        ...
        const ratio = BigInt(poolSize) / (totalK /  BigInt(ksmDecimals))
        setRatioReward(parseInt(ratio.toString(10)))
        buffList = buffList.sort((a, b) => (a.total < b.total) ? 1 : -1)
        let showList: any[] = []
        buffList.forEach(val => {
            if (val.total / ksmDecimals > 400) {
                return;
            }
    ...
    useEffect(() => {
        getContribution(currentWallet).then()
        if (currentWallet) {
                        ...
                        <div className={styles.headerText}>
                            <p><span className={styles.headerText1}>
                                ...
                                <span style={{display: isMobile ? "block" : "inline"}}> 
                                    <img alt="Polka logo" src={polkaLogo}/><b> Polka</b>Smith
                                </span>
                                <b> on Kusama Parachain Auction</b>
                            </span></p>
                            ...
                            <p className={styles.headerContent}>
                                Participants who support PolkaSmith to win the Kusama auction will earn worthy rewards in PKS. In addition, there are an extra 10% PKS for early birds who contribute in first 7 days.
                            </p>
                            <button className={styles.joinBTN} style={{float: "left"}} onClick={() => {
                                executeScroll()
                            }}>
                                JOIN NOW
                                <img  alt="Join now" src={arrowRightIcon} style={{marginLeft: 5}}/>
                            </button>
                        </div>
                        <div className={styles.headerImg}>
                            <img  alt="header" src={headerImg} width={"100%"}/>
                        </div>
                    </div>
                    <div className={styles.overviewLabel}>
                        <div className={styles.label}>
                            <h3 style={{color: "#aeaeae"}}>Token to Contribute</h3>
                            <h2>KSM</h2>
                        </div>
                        ...
                        <div className={styles.label} style={{textAlign: "center"}}>
                            ...
                            <p>{isLoadingContributed ?
                                <img  alt="Loading" src={loading} width={25} height={25}/> :
                                <span><span
                        ...
                        <div className={styles.label} style={{textAlign: "right"}}>
                            <h3 style={{color: "#aeaeae"}}>Reward Pool</h3>
                    ...
                    <div ref={myRef} className={styles.introMain}>
                            ...
                            <div className={styles.introBlock}>
                                <div className={styles.introContribute}>
                                    ...
                                    <a style={{float: "right", color: "#6398FF", fontSize: 16}}
                                       href="https://polkadot.js.org/extension/" target={"_blank"} rel="noreferrer">Get Polkadot.js
                                        extension?</a>
                                    {!currentWallet ?
                                        <div style={{marginTop: 50, textAlign: "left"}}>
                                            ...
                                            <h3>{formatNumber(ksmAmountCal * 500, 2, false)} Red Kite Points</h3>
                                        </div> :
                                        ...
                                        <div style={{
                                            ...
                                            <div className={styles.detailInfo}>
                                                ...
                                                <h2>{isLoadingContributed ?
                                                    <img  alt="Loading" src={loading} width={25} height={25}/> :
                                                    formatNumber(contributed / ksmDecimals, 2, false)} KSM</h2>
                                                <h3 style={{marginTop: 20, color: "#aeaeae"}}>Estimated Rewards</h3>
                                                <h2>{isLoadingContributed ?
                                                    <img alt="Loading" src={loading} width={25} height={25}/> :
                                                    formatNumber(contributed * ratioReward / ksmDecimals, 0, false)} PKS</h2>
                                                <h2>{isLoadingContributed ?
                                                    <img alt="Loading" src={loading} width={25} height={25}/> :
                                                    formatNumber(contributed * 500 / ksmDecimals, 0, false)} RKPoints <img alt="Info" src={infoIcon} width={15} height={15} ref={setTriggerRef}/></h2>
                                                {visible && (
                                                    <div
                                                        ref={setTooltipRef}
                                                        {...getTooltipProps({ className: 'tooltip-container' })}
                                                    >
                                                        Red Kite Point is the “equivalent PKF” token on our launchpad
                        ...
                        <Collapse isOpen={currentWallet !== null}>
                            <div className={styles.contributeForm}>
                                {successStatus ?
                                    <h1><img alt="Success" src={success} width={30} height={30}/> You have successfully contributed
                                    </h1> :
                                ...
                                <div className={styles.contributeInputGroup}>
                                    ...
                                    <div className={styles.contributeInput}>
                                        <h2 style={{
                                            ...
                                            width: "50%",
                                            textAlign: "left"
                                        }}>{ isDue ? "0.00" : <span>{ ksmAmount > 0 && isLoadingContributed ?
                                            <img alt="Loading" src={loading} width={20} height={20}/> :
                                            formatNumber(ksmAmount * ratioReward * 0.1, 2, false)}</span>} PKS</h2>
                                        <div style={{display: "inline-block", width: "50%", textAlign: "right"}}>
                                            <h4>(10%)</h4></div>
                                    </div>
                                </div>
                                <div className={styles.contributeInputGroup}>
                                    ...
                                    <div className={styles.contributeInput}>
                                        <h2 style={{
                                            ...
                                            width: "50%",
                                            textAlign: "left"
                                        }}>{formatNumber(ksmAmount * 500, 2, false)} Points</h2>
                                        <div style={{display: "inline-block", width: "50%", textAlign: "right"}}>
                                            <h4>(1 KSM : 500 Points)</h4></div>
                                    </div>
                                </div>
                                <div className={styles.contributeInputGroup}>
                                    <div className={styles.contributeInputLabel}>
                                ...
                                <Collapse isOpen={!!contributionHash}>
                                    <div className={styles.contributeInputGroup}>
                                        <div className={styles.contributeInputLabel}>
                                        ...
                                        <div className={styles.contributeInput}>
                                            <h3><a target={"_blank"}
                                                   ...
                                                   style={{color: "#6398FF"}} rel="noreferrer"> {truncateAddress(contributionHash, 13)} </a>
                                            </h3>
                                        </div>
                                    </div>
                                </Collapse>
                            ...
                            <span>I have read and accepted the <a href={"/#/polkasmith-privacy"} target={"_blank"}
                                                                  style={{color: "#6398FF"}} rel="noreferrer"><b>Term and Conditions</b></a> as well as the <a href={"https://redkite.polkafoundry.com/#/privacy"} target={"_blank"} style={{color: "#6398FF"}} rel="noreferrer">Privacy Policy</a>.
                            I agree to receive email communications about PolkaSmith and PolkaFoundry, including exclusive launch updates and liquidity provider programs.</span>
                                ...
                                { !isSubmitting ? <div/> :
                                    <div className={styles.statusMessage}>
                                        <div className={styles.statusTitle}><span>{ statusMessage }</span></div>
                                        <div className={styles.statusTime}><span>{ executeTime }</span></div>
                                    </div>}
                                {successStatus ?
                                    <button className={styles.connectWallet} onClick={contributeMore}>CONTRIBUTE
                                        MORE <img alt="more" src={arrowRightIcon} style={{marginLeft: 5}}/></button> :
                                    <button className={styles.connectWallet} onClick={submitContribution}
                                            disabled={true || isSubmitting || !erc20Wallet.isValid || ksmAmount < 0.1 || ksmAmount > ksmBalance.unlocked || !agreePolicy}>
                                        {!isSubmitting ? "Submit Contribution" :
                                            <img alt="submit" src={loading} width={30} height={30}/>}
                                    </button>}
                            </div>
                        </Collapse>
                        <h3>📌 After KSM contribution, 100% Red Kite point delivered immediately</h3>
                        <h3>📌 After PolkaSmith wins, 35% of PKS delivered immediately and 65% PKS vested over 10 months</h3>
                    </div>
                    <div className={styles.additionalContainer}>
                        <h1>KSM Contribution</h1>
                        <div className={styles.additionalInfo}>
                            ...
                            <div className={styles.additionalLabelContainer}
                                ...
                                <div className={styles.additionalLabel}>
                                    ...
                                    <h2>{isLoadingContributed ?
                                        <img alt="Loading" src={loading} width={25} height={25}/> :
                                        formatNumber(parseFloat((totalKSM / BigInt(ksmDecimals)).toString(10)), 0, true)} </h2>
                                </div>
                            </div>
                            <div className={styles.additionalLabelContainer}>
                                <div className={styles.additionalLabel}>
                            ...
                            <div className={styles.leaderBoardTable}>
                                {
                                    !isLoadingContributed ?
                                        ...
                                        alt="Loading" src={loading} width={50} height={50}/></div>
                                }
                            </div>
                        </div>
                    </div>
