...
const Dashboard = (props: any) => {
  ...
  const [upcomingCommunityPoolsV3Display, setUpcomingCommunityPoolsV3Display] = useState<Array<any>>([]);
  ...
  const { data: upcomingPoolsV3, loading: loadingUpcomingPoolV3 } = useFetch<PoolData>(`/pools/v3/upcoming-pools`);
  const { data: activePoolsV3, loading: loadingActivePoolsV3 } = useFetch<PoolData>(`/pools/v3/active-pools`);
  const { data: nextToLaunchPoolsV3, loading: loadingNextToLaunchPoolsV3 } = useFetch<PoolData>(`/pools/v3/next-to-launch-pools`);
  const { data: completeSalePoolsV3, loading: loadingCompleteSalePoolsV3 } = useFetch<PoolData>(`/pools/v3/complete-sale-pools`);
  ...
  useEffect(() => {
    ...
    if (upcomingPoolsV3?.data && upcomingPoolsV3.data.length) {
      ...
      setStatusPools(pools)
        .then(() => {
          setUpcomingCommunityPoolsV3Display(pools.filter((p: any) => p?.is_private === 3))
          setUpcomingPoolsV3Display(pools.filter((p: any) => p?.is_private !== 3));
        });
    }
  ...
  return (
    <DefaultLayout>
      ...
      {((upcomingPoolsV3Display && upcomingPoolsV3Display.length > 0) || (upcomingCommunityPoolsV3Display && upcomingCommunityPoolsV3Display.length > 0)) &&
      <div className={styles.listPools}>
        <h2>Upcoming</h2>
        {
          ...
          <>
            <h3>POOL IDO</h3>
            <div className="pools">
              {upcomingPoolsV3Display.filter(p => p.start_join_pool_time).map((pool: any, index) => {
        ...
        {
          upcomingCommunityPoolsV3Display && upcomingCommunityPoolsV3Display.length > 0 &&
          <>
            <h3 style={{marginTop: '20px'}}>POOL COMMUNITY</h3>
            <div className="pools">
              {upcomingCommunityPoolsV3Display.filter(p => p.start_join_pool_time).map((pool: any, index) => {
                  return <Card pool={pool} key={pool.id} autoFetch={true} />
                })}
              {upcomingCommunityPoolsV3Display.filter(p => !p.start_join_pool_time).map((pool: any, index) => {
                return <Card pool={pool} key={pool.id} autoFetch={true} />
      ...
      <div className={styles.getAlert}>
        <div className={styles.contentGetAlert}>
          <h2 className={styles.titleGetAlert}>Want to launch your project <br/> on Red Kite?</h2>
          <button className={styles.btnGetAlert} onClick={() => window.open('https://t.me/PolkaFoundryANN', '_blank')}>
            Apply to Launch&nbsp;
            <img src="/images/icons/icon_get_alert.svg"  alt=""/>
          </button>
        </div>
      </div>
    </DefaultLayout>
