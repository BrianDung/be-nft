    ...
    try {
      if (userAddress && poolAddress && tokenDetails
        ...
        if (contract) {
          ...
          const userPurchasedReturn = new BigNumber(userPurchased).minus(new BigNumber(userClaimed)).div(new BigNumber(10).pow(tokenDetails.decimals || 0)).toFixed();
          
          return {
            userPurchased: new BigNumber(userPurchased).div(new BigNumber(10).pow(tokenDetails.decimals || 0)).toFixed(),
            userClaimed: new BigNumber(userClaimed).div(new BigNumber(10).pow(tokenDetails.decimals || 0)).toFixed(),
            userPurchasedReturn,
          }
        }
        return {
