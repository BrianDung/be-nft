...
export type poolDetailKey = Extract<
  ...
  PoolDetailKey.startRegisterTime |
...
const usePoolDetailsMapping = (poolDetails: PoolDetailMappingProps | undefined): PoolDetailMapping | undefined => {
  if (poolDetails) {
    ...
    const joinTimeInDate = unixToDate(startRegisterTime);
    const endJoinTimeInDate = unixToDate(endRegisterTime);
    const startBuyTimeInDate = unixToDate(startBuyTime);
    const endBuyTimeInDate = unixToDate(endBuyTime);
    ...
    const poolDetailsBasic = {
      ...
      [PoolDetailKey.minTier]: {
        display: minTier === 0 ? (kycBypass ? 'Min Tier TBD & KYC required' : 'Min Tier TBD') : getUserTierAlias(minTier).text,
        label: 'Min Tier',
      ...
      [PoolDetailKey.startRegisterTime]: {
        display: `${startRegisterTime ? convertTimeToStringFormatWithoutGMT(joinTimeInDate) : 'TBA'}  -  ${endRegisterTime ? convertTimeToStringFormat(endJoinTimeInDate) : 'TBA'}`,
