...
export default (props: PoolTimeLineProps) => {
  const { currentStatus, display, poolDetails, countDownDate } = props;
  ...
  const statusBarSteps: { name: string, value: string, active: string, tooltip: string }[] = [
    {
      ...
      tooltip: `This status is displayed before the pool is opened for swapping. 
                ...
                ${`Registration begins: <span>${joinTimeInDate ? convertTimeToStringFormat(joinTimeInDate) : 'TBA'}</span>`}`,
    },
    {
      name: 'Swap',
      value: '2',
      ...
      tooltip: `You can start buying Tokens when the pool is changed to Swap status.<br/> 
                ${`Token Swap Time: <span>${startBuyTimeInDate ? convertTimeToStringFormat(startBuyTimeInDate) : 'TBA'}</span>`}`,
    },
    {
      name: 'Filled',
      value: '3',
    ...
    },
    {
      ...
      <span>${releaseTimeInDate ? `Token claim time: ${convertTimeToStringFormat(releaseTimeInDate)}` : `Token Claim time will be annouced soon.`}</span>`
    },
    {
      name: 'End',
      value: '5',
