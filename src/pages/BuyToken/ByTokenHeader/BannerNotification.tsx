...
import {ACCEPT_CURRENCY, PUBLIC_WINNER_STATUS, TIER_LEVELS, TIERS, POOL_IS_PRIVATE} from "../../../constants";
...
function BannerNotification(props: any) {
  ...
  const isInPreOrderTime = poolDetails?.startPreOrderTime && startBuyTimeInDate && new Date(poolDetails.startPreOrderTime * 1000) < now && now < startBuyTimeInDate;
    ...
    <>
      {poolDetails && !isKYC && connectedAccount &&
        <div className={styles.alertVerifyEmail}>
          <img src="/images/warning-red.svg" style={{ marginRight: "8px" }} alt="" />
          <span>
      ...
      {isKYC && currentUserTier?.level < poolDetails?.minTier && !isOverTimeApplyWhiteList &&
      ...
      {isInPreOrderTime && alreadyJoinPool && currentUserTier && currentUserTier.level === TIER_LEVELS.MAXIMALS &&
      <div className={styles.warningWhite}>
        <img src="/images/warning-white.svg" style={{ marginRight: "10px" }} alt="" />
        {
      ...
      {isInBuying && alreadyJoinPool && !isInFreeBuying && existedWinner && poolDetails?.campaignStatus !== PoolStatus.Filled &&
        <div className={styles.warningWhite}>
          <img src="/images/warning-white.svg" style={{ marginRight: "10px" }} alt="" />
          {
            purchasableCurrency.toUpperCase() === ACCEPT_CURRENCY.ETH?.toUpperCase()
      ...
      (alreadyJoinPool || joinPoolSuccess) && !(currentUserTier && currentUserTier.level === TIER_LEVELS.MAXIMALS && isInPreOrderTime) &&
      ...
      { alreadyJoinPool && !!poolDetails?.freeBuyTimeSetting?.start_buy_time && isInFreeBuying && poolDetails?.campaignStatus !== PoolStatus.Filled &&
        <div className={styles.warningWhite}>
          <span>
            Free Time Purchase (Phase 2) has started!
          </span>
        </div>
      }

      { alreadyJoinPool && !!poolDetails?.freeBuyTimeSetting?.start_buy_time && isInFreeBuying && poolDetails?.campaignStatus !== PoolStatus.Filled && new BigNumber(userPurchasedUsdt).eq(maximumBuy) &&
        <div className={styles.warningWhite}>
          <span>
                You have reached your&nbsp;
            <span style={{color: '#ff673e'}}>{numberWithCommas(`${userBuyLimit}`)} {currencyName}</span>&nbsp;
      ...
      { !alreadyJoinPool && isOverTimeApplyWhiteList && poolDetails?.campaignStatus !== PoolStatus.Closed &&
      <div className={styles.warningWhite}>
            <span>
              {Number(poolDetails?.isPrivate || '0') === POOL_IS_PRIVATE.COMMUNITY ? 'Whitelist / Competition period has ended.' : 'Whitelist application period is closed.'}
