...
export const getPoolCountDown = (
  ...
  const isNextLaunch = poolStatus === PoolStatus.Joining;
  const isSwaping = poolStatus === PoolStatus.Progress;
  const isPublicSell = poolStatus === PoolStatus.PublicSell;
  ...
  const endPublicSellTime = formatTimeInDate(poolDetails?.freeBuyTimeSetting.end_buy_time);
  
  // if (method && method === "whitelist") {
    if (isUpcoming || isNextLaunch) {
      if (startJoinTime && today < startJoinTime.getTime()) {
        date = startJoinTime;
        display = 'You Can Register In  ';
        displayShort = isCommunityPool
          ? "Competition starts in"
          : "Whitelist starts in";
        return { date, display, displayShort };
      } else if (startJoinTime && endJoinTime && startJoinTime.getTime() < today && today < endJoinTime.getTime() && !alreadyJoinPool && !joinPoolSuccess ) {
        date = endJoinTime;
        if (isCommunityPool) {
          display = "Registration Ends In";
    ...
    } else if(isPublicSell &&  new BigNumber(soldProgress).lt(100)){
        const date = endPublicSellTime;
        display = 'Public Sell Ends In';
        return { date, display, displayShort };
    } else if (
